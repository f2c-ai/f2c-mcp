...
describe('Full Integration Tests - MCP + Socket', () => {
  ...
  const SOCKET_URL = process.env.HONO_WS_URL || 'ws://localhost:3000/ws'
  ...
  describe('2. Socket Connection Tests', () => {
    ...
    it('should handle Socket request-response', async () => {
      ...
      try {
        ...
        const response = await socketClient.request<typeof testData, { content: string }>(
          'get_html_content',
          testData
        )
    ...
    it('should handle Socket timeout gracefully', async () => {
      ...
      try {
        ...
        const isTimeoutOrConnection = error.message.includes('timeout') || 
  ...
  describe('6. Error Handling Tests', () => {
    ...
    it('should provide meaningful error messages', async () => {
      ...
      // 检查是否包含有用的信息
      const hasUsefulContent = text.includes('Convert') || 
                              text.includes('ErrorMessageTest') ||
